...
- **0.62.0** recovers the time of work whose session ran in a working copy that was since removed: its transcript is found by the task it names. A problem found during setup is filed beside the setup task instead of under it, so the setup can still be accepted. And an earlier session that has stopped is no longer reported as one waiting for your answer: the agent takes its work over.
- **0.61.0** makes the time check bind only work begun after time records came into a project. Rerunning setup lists the work that was already in flight then, and where the transcript of the session that took an item is on the machine, it writes that session's claim at the start the transcript shows, marked recovered, and then its time. Work in flight is no longer an error that blocks the upgrade. Projects rerun setup once.
- **0.60.0** keeps everything about how the work went in the tracker, on the item it is about, and nothing on a machine: a session's claim carries the forecast by phase and the promised time, and when it stops working on the item a script writes what it spent, by phase and by who spent it, measured from its transcript. Stops, decisions, the feature's summary (forecast against work, time occupied), your verdict and handoff notes for work with no feature are records there too, so every machine and session reads the same history. The backlog check finds time that was never written, and reports say "at least" while any is open. Projects rerun setup once.
- **0.59.1** keeps a feature run from starting CI before its pull request is ready: a stage's full checks run on the agent's machine, the pull request stays in progress while review fixes are merged, and CI runs once when everything is in. A red CI job is reproduced locally before any fix is pushed.
