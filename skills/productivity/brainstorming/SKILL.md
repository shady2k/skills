...
protocol's **A decision the owner gives is kept** — through `to-backlog` where
the project has a backlog integration, and where it has none, said plainly as
something that will be lost unless the user keeps it himself.
...
Take the user's role as the protocol's **Speaking to the owner** says; ask
about it only if it changes who should decide.
...
**A decision given is not reopened** and **A message that changes no next
action is not sent** hold.
